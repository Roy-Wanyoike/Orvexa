...
func TestAuthzMatrix(t *testing.T) {
	...
	l.probeNoLeak(s, "R1", r1, m1, "tenant-B key (own resource control)",
		...
		fmt.Sprintf(`{"display_name":"B control","identifiers":[{"type":"phone","value":"2547%s"}]}`, fx.runID+"b1"),
		needles, 201)
	l.probe(s, "R1", r1, m1, "tenant-A key", http.MethodPost, "/api/v1/customers/", fx.keyA,
		fmt.Sprintf(`{"display_name":"A create probe","identifiers":[{"type":"phone","value":"2547%s"}]}`, fx.runID+"a1"), nil, 201)
	...
	l.probeNoLeak(s, "R9", r9, m9, "tenant-B key on tenant-A conversation", http.MethodGet, "/api/v1/conversations/"+fx.convA1+"/interactions", fx.keyB, "", needles, 404)
	...
	l.probeNoLeak(s, "R10", r10, m10, "tenant-B key (own resource control)",
		...
		fmt.Sprintf(`{"customer_id":%q,"channel":"chat","direction":"inbound","source":"matrix:b","destination":"queue-b"}`, fx.custB1),
		needles, 201)
	l.probe(s, "R10", r10, m10, "tenant-A key", http.MethodPost, "/api/v1/interactions/", fx.keyA,
		fmt.Sprintf(`{"customer_id":%q,"channel":"chat","direction":"inbound","source":"matrix:a","destination":"queue-a"}`, fx.custA1), nil, 201)
	...
	l.probeNoLeak(s, "R25", r25, m25, "tenant-B key on tenant-A case", http.MethodGet, "/api/v1/cases/"+fx.caseA1+"/notes", fx.keyB, "", append(needles, "matrix-note-A2"), 404)
	...
	l.probe(s, "R27", r27, m27, "tenant-A key", http.MethodPost, "/api/v1/calls/", fx.keyA,
		fmt.Sprintf(`{"customer_id":%q,"to":"+254711111112"}`, fx.custA1), nil, 201)
	...
	l.probe(s, "R28", r28, m28, "tenant-A key", http.MethodPost, "/api/v1/calls/"+fx.interA2+"/actions", fx.keyA, `{"action":"hold"}`, nil, 200)
	...
	l.probe(s, "IDW-3", "GET /api/v1/cases/{A-id}/notes", "IDOR: real tenant-A UUID via tenant-B key",
		"tenant-B key", http.MethodGet, "/api/v1/cases/"+fx.caseA2+"/notes", fx.keyB, "", nil, 404)

	// reference-in-body probes: cross-tenant UUIDs in BODIES of B-key creates
	refBody := func(custID string) string {
		return fmt.Sprintf(`{"customer_id":%q,"channel":"chat","direction":"inbound","source":"matrix:ref","destination":"queue-b"}`, custID)
	}
	...
	l.probeDefect(s, "REF-2", "POST /api/v1/calls/ (tenant-A customer_id in body)", m27,
		...
		defectByID("D1"), 201, 404, 422)
	l.probeDefect(s, "REF-3", "POST /api/v1/messages/ (tenant-A customer_id in body)", m29,
