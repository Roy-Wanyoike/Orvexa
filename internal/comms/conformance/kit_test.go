...
// is the REFERENCE behavior — it must pass both contracts green, so the
// kit's assertions and the reference can never drift apart silently. Two
// additional in-process fakes (an async/callback-style voice adapter with
// full command validation, and a re-validating messenger) prove the Options
// knobs are live wiring, not dead flags.
...
// validatingMessenger re-validates every Send with the core gate (reusing
// the exact ValidateMessage codes) and delivers receipts synchronously —
// the contract behind Options.EnforceSendValidation.
type validatingMessenger struct {
	rec *conformance.Recorder
}

func (v *validatingMessenger) Send(_ context.Context, msg messaging.Message) error {
	...
	for _, ev := range []comms.ProviderEvent{
		{Event: "message.sent", InteractionID: msg.InteractionID, TenantID: msg.TenantID},
		{Event: "message.delivered", InteractionID: msg.InteractionID, TenantID: msg.TenantID},
	} {
		body, err := ev.Encode()
		if err != nil {
			return err
		}
		if err := v.rec.Ingest(context.Background(), "validating-messenger", body, testSigner(body)); err != nil {
			return err
		}
	}
	return nil
...
// TestSimulatorPassesMessagingConformance dogfoods the messaging contract
// against the reference Simulator.
func TestSimulatorPassesMessagingConformance(t *testing.T) {
	rec := conformance.NewRecorder()
	sim := comms.NewSimulator(rec.Ingest, testSigner, 0)
	conformance.RunMessagingConformance(t, sim, conformance.Options{
		Recorder:     rec,
		ProviderName: "simulator",
	})
}

// TestValidatingAdapterPassesMessagingConformance proves EnforceSendValidation
// is live: the re-validating adapter must survive the full messaging contract
// with the enforcement flag on, including the exact-code checks.
func TestValidatingAdapterPassesMessagingConformance(t *testing.T) {
	rec := conformance.NewRecorder()
	conformance.RunMessagingConformance(t, &validatingMessenger{rec: rec}, conformance.Options{
		Recorder:              rec,
		ProviderName:          "validating-messenger",
