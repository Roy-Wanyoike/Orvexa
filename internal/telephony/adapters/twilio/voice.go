...
// mintSID is used by the test fake (fake_test.go) to produce Twilio-shaped
// call sids (CA + 32 hex). Kept next to the adapter's resource paths so the
// fake mirrors the identifier shape the adapter round-trips.
func mintSID() string {
	var b [16]byte
